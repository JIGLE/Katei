  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 pb-safe">
      <div className="flex items-center justify-center px-3 pb-4">
        <div className="flex items-center gap-1 rounded-full border border-zinc-800/60 bg-zinc-900/95 px-1.5 py-1.5 shadow-2xl backdrop-blur-sm">
          {visibleTabs.map((tab) => (
            <NavLink
              key={tab.to}
              to={tab.to}
              ...
              className={({ isActive }) =>
                [
                  'relative flex min-w-11 flex-col items-center gap-1.5 rounded-xl px-1.5 py-1.5 text-[0.7rem] transition-colors duration-150',
                  isActive

        ...
        {visibleTabs.map((tab) => (
          ...
          >
            {({ isActive }) => (
              <>
                {tab.icon}
                {/* Below 360px (narrower than any current phone) all six
                    locales would ellipsise, so drop to icons rather than show
                    five clipped words. Above that, truncate is a safety net
                    only — no current label needs it, but it means a future
                    translation can never push the bar off-screen again. */}
                <span className="max-w-full truncate max-[359px]:hidden">{t(tab.labelKey)}</span>
                {tab.accent && isActive && (
                  <span
                    className={`absolute right-1.5 top-1.5 h-1 w-1 rounded-full ${accentColor[tab.accent]}`}
                  />
                )}
              </>
            )}
